...
export const ALL_POSTS_QUERY = gql`
  query findStories($country: string!,$topicid : string!, $variantIds: string!) {
...
export default function TopStoriesVideos () {
  ...
  const { findStories, currentPage, totalPages } = data
  const totposts = findStories.stories.length

  return(
    <fragment>
      <div className="row">
        {findStories.stories.map(({ bannerUrls,videoUrls,videoUrlsGroup,title,customer,viewsCount,dislikesCount,likesCount,commentsCount,sharesCount },index) => (
...
class TopStoriesVideos1 extends Component {
